(function (root, factory) {
    ...
    renderDay = function(opts, date, dummy, extraClass)
    {
        ...
        if (opts.minDate && date.isBefore(opts.minDate, 'day')) {
            day.className.push('is-disabled');
            day.className.push('less-min-date');
        }
    ...
    renderCalendar = function(el, opts)
    {
        ...
        if (opts.ranges) {
			...
			Object.keys(opts.ranges).forEach(function (rangeLabel) {
				...
				html += '<div class="lightpick__ranges-select-link'+(range[0]&&range[0].isBefore(opts.minDate, 'day') ? ' less-min-date' : '')+'" data-range-start="' + range[0] + '" data-range-end="' + range[1] + '">' + rangeLabel + '</div>'
			});
			html += '</div>';
			html += '</section>';
		}
    ...
    Lightpick = function(options)
    {
        ...
        self._onMouseDown = function(e)
        {
            ...
            // Add ranges
			if (target.classList.contains('lightpick__ranges-select-link') && !target.classList.contains('less-min-date')) {
				self.setStartDate(moment(parseInt(target.dataset['rangeStart'])), true);
				...
				_.each(self.el.querySelectorAll('.lightpick__ranges-select-link'), function (l) {
                    if(l == target) {
			    ...
			    var rangesLink = self.el.querySelectorAll('.lightpick__ranges-select-link');
                _.each(rangesLink, function (l, i) {
                    if(i == rangesLink.length-1) {
                        l.classList.add('active');
                    } else {
                        l.classList.remove('active');
                    }
    ...
    Lightpick.prototype = {
        ...
        {
            var opts = _.extendOwn({}, defaults, options);
            ...
            if (Object.prototype.toString.call(options.locale) === '[object Object]') {
                opts.locale = _.extendOwn({}, defaults.locale, options.locale);
        ...
        setEndDate: function(date, preventOnSelect)
        {
            ...
            if (!preventOnSelect && typeof this._opts.onSelectEnd === 'function') {
